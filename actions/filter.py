"""Provides the filter action object."""
...
class FilterAction(action.Action):
    """This class provides the mechanism for performing filter actions.

    The filter action filter downloads in the sense that it looks for duplicate feed downloads and removes them. If following a strategy of
    downloading feeds at a fixed interval of time, as the download action does, it is likely that there will be many duplicates, especially
    if downloading frequently for redundancy. 

    The filter action iterates through each file in the downloaded directory. For each file it tries to interpret it (given the file name)
    as a feed download, if it can't the file is deleted. For valid downloads, the action attempts to read the file using the user provided
    timestamp function. The timestamp function takes the path of the file and returns the time the feed was generated by the transit authority.
    If the function returns -1, or if an exception is raised, that download is deemed corrupt and deleted. Otherwise the file is copied to
    the filtered directory, with a new file name based on the transit authority timestamp. Duplicate files are easily discovered as the 
    transit authority timestamp, and hence the new file name, are the same.
    
    To use the download action, initialize in the common way for all actions:

        action = FilterAction(root_dir=, feeds=, quiet=, log_file_path=)
    ...
    Attributes:
        limit (int): an integer imposing an upper bound on the number of downloaded files to process. If set to -1, no limit is imposed. Default is -1.
        n_total (int): number of downloaded files processed.
        n_corrupt (int): number of downloaded files that were deemed corrupt.
        n_copied (int): number of downloaded files copied to the filtered directory.
        n_skipped (int): number of downloaded files that were not copied because they were duplicated of files already copied.
    """
