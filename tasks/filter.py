...
class FilterTask(task.Task):
    ...
    The filter task filter downloads in the sense that it looks for duplicate feed downloads and removes them. If following a strategy of
    downloading feeds at a fixed interval of time, as the download task does, it is likely that there will be many duplicates, especially
    if downloading frequently for redundancy. 

    The filter task iterates through each file in the downloaded directory. For each file it tries to interpret it (given the file name)
    as a feed download, if it can't the file is deleted. For valid downloads, the task attempts to read the file using the user provided
